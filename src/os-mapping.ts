...
import { OpenSeaSale, Token, TokenOpenSeaSaleLookupTable } from "../generated/schema";
import { ARTBLOCKS_ADDRESS, ARTBLOCKS_ORIGINAL_ADDRESS, WYVERN_ATOMICIZER_ADDRESS } from "./constants";
...
function _handleSingleAssetSale(call: AtomicMatch_Call): void {
  ...
  if (nftContract.toHexString() == ARTBLOCKS_ORIGINAL_ADDRESS || nftContract.toHexString() == ARTBLOCKS_ADDRESS) {
    ...
    if (token != null) {
      ...
      let tokenOpenSeaSaleLookupTable = new TokenOpenSeaSaleLookupTable(tableEntryId);
...
function _handleBundleSale(call: AtomicMatch_Call): void {
  ...
  if(nftContractList.includes(ARTBLOCKS_ORIGINAL_ADDRESS) || nftContractList.includes(ARTBLOCKS_ADDRESS)) {
 ...
 */
function _getNftContractAddressAndTokenIdFromCallData(atomicizeCallData: string): string[][] {
  const TRAILING_0x = 2;
