...
export function handleOrdersMatched(event: OrdersMatched): void {
  ...
  if (saleWrapper != null) {
    ...
    let numSalesPerUniqueProjectId: BigInt[] = [];
    for (let i = 0; i < associatedProjectsIds.length; i++) {
      ...
      if (index == -1) {
        ...
        numSalesPerUniqueProjectId.push(BigInt.fromI32(1));
      } else {
        numSalesPerUniqueProjectId[index] = numSalesPerUniqueProjectId[
          index
    ...
    for (let i = 0; i < uniqueProjectsIds.length; i++) {
      ...
      let numSalesForThisProject = numSalesPerUniqueProjectId[i];
      ...
      if (project != null) {
        ...
        let pricePart = event.params.price
          .times(numSalesForThisProject)
          .div(nbTransfersInSale);
