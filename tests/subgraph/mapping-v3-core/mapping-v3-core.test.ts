...
test(`${coreType}: Handles PlatformUpdated::dependencyRegistryAddress - changed value to valid dependency registry`, () => {
  clearStore();
  // add new contract to store
  const projectId = BigInt.fromI32(0);
  addTestContractToStore(projectId);
...
test(`${coreType}: Handles PlatformUpdated::dependencyRegistryAddress - changed value to invalid dependency registry`, () => {
  clearStore();
  // add new contract to store
  const projectId = BigInt.fromI32(0);
  addTestContractToStore(projectId);
...
describe(`${coreType}: handleProjectUpdated`, () => {
  describe("create", () => {
    ...
    test("should create a project entity", () => {
      ...
      // default royalty upon project creation is 0 for flagship V3 core
      assert.fieldEquals(
        PROJECT_ENTITY_TYPE,
        generateContractSpecificId(TEST_CONTRACT_ADDRESS, projectId),
        "royaltyPercentage",
      ...
      // project render provider royalties should equal contract-level values for flagship V3 core
      assert.fieldEquals(
        PROJECT_ENTITY_TYPE,
        generateContractSpecificId(TEST_CONTRACT_ADDRESS, projectId),
        "renderProviderSecondarySalesAddress",
      ...
      // flagship: platform royalties are null
      assert.fieldEquals(
        PROJECT_ENTITY_TYPE,
        generateContractSpecificId(TEST_CONTRACT_ADDRESS, projectId),
